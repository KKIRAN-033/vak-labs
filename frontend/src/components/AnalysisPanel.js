...
export default function AnalysisPanel({ analysisData, selectedDataset, loading, apiUrl }) {
  ...
  const topPairsChart = useMemo(() => {
    return (analysis.top_pairs || []).slice(0, 6).map((p) => ({
            ...
            {activeTab === \"overview\" && (
              <>
                <div className=\"grid grid-cols-2 lg:grid-cols-4 gap-3\" data-testid=\"stats-grid\">
                  <StatCard icon={FileText} label=\"Total Records\" value={totalRecords.toLocaleString()} sub={analysisData?.dataset_type || \"\"} color=\"text-primary\" />
                  <StatCard icon={Users} label=\"Active Entities\" value={(analysis.top_entities || []).length} sub=\"Tracked\" color=\"text-emerald-400\" />
                  <StatCard icon={Moon} label=\"Night Activity\" value={`${analysis.night_activity?.percentage || 0}%`} sub={`${analysis.night_activity?.count || 0} records`} color=\"text-amber-400\" />
                  <StatCard icon={AlertTriangle} label=\"Suspects\" value={suspicious.length} sub={suspicious.filter((s) => s.severity === \"HIGH\").length + \" high severity\"} color=\"text-red-400\" />
                </div>
                      ...
                      <CardContent className=\"p-4 pt-0\">
                        <ResponsiveContainer width=\"100%\" height={180}>
                          <BarChart data={timelineData}>
                            ...
                            <XAxis dataKey=\"hour\" tick={{ fontSize: 9, fill: \"hsl(240 5% 65%)\" }} interval={2} axisLine={{ stroke: \"hsl(240 4% 16%)\" }} />
                            <YAxis tick={{ fontSize: 9, fill: \"hsl(240 5% 65%)\" }} axisLine={{ stroke: \"hsl(240 4% 16%)\" }} />
                            <RechartsTooltip contentStyle={{ background: \"hsl(240 5% 8%)\", border: \"1px solid hsl(240 4% 16%)\", borderRadius: \"6px\", fontSize: \"11px\", color: \"hsl(0 0% 98%)\" }} />
                  ...
                  {entityPieData.length > 0 && (
                    <Card className=\"border-border/40 bg-card/60\" data-testid=\"entities-chart\">
                      ...
                      <CardContent className=\"p-4 pt-0\">
                        <ResponsiveContainer width=\"100%\" height={180}>
                          <PieChart>
                            <Pie data={entityPieData} cx=\"50%\" cy=\"50%\" innerRadius={45} outerRadius={70} paddingAngle={2} dataKey=\"value\">
                              {entityPieData.map((_, idx) => (
                                <Cell key={idx} fill={CHART_COLORS[idx % CHART_COLORS.length]} />
                              ))}
                            </Pie>
                            <RechartsTooltip formatter={(val, name, props) => [val, props.payload.full || name]} contentStyle={{ background: \"hsl(240 5% 8%)\", border: \"1px solid hsl(240 4% 16%)\", borderRadius: \"6px\", fontSize: \"11px\", color: \"hsl(0 0% 98%)\" }} />
                          </PieChart>
                        </ResponsiveContainer>
                      </CardContent>
                    </Card>
                    ...
                    <CardContent className=\"p-4 pt-0\">
                      <ResponsiveContainer width=\"100%\" height={160}>
                        <BarChart data={topPairsChart} layout=\"vertical\">
                          ...
                          <XAxis type=\"number\" tick={{ fontSize: 9, fill: \"hsl(240 5% 65%)\" }} axisLine={{ stroke: \"hsl(240 4% 16%)\" }} />
                          <YAxis dataKey=\"name\" type=\"category\" width={80} tick={{ fontSize: 9, fill: \"hsl(240 5% 65%)\", fontFamily: \"JetBrains Mono\" }} axisLine={{ stroke: \"hsl(240 4% 16%)\" }} />
                          <RechartsTooltip formatter={(val, name, props) => [val, props.payload.full]} contentStyle={{ background: \"hsl(240 5% 8%)\", border: \"1px solid hsl(240 4% 16%)\", borderRadius: \"6px\", fontSize: \"11px\", color: \"hsl(0 0% 98%)\" }} />
                        ...
                        <TableBody>
                          {(analysis.unique_connections || []).slice(0, 8).map((c, i) => (
                            <TableRow key={i}>
                              ...
                              <TableCell className=\"text-right font-mono text-xs py-1.5\">{c.unique_contacts}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
                        ...
                        <TableBody>
                          {(analysis.movement_patterns || []).map((m, i) => (
                            <TableRow key={i}>
                              ...
                              <TableCell className=\"text-right font-mono text-xs py-1.5\">{m.towers_visited}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
                ...
                {suspicious.length === 0 ? (
                  ...
                  <>
                    <div className=\"flex items-center gap-2 mb-2\">
                      <span className=\"text-[10px] uppercase tracking-wider text-muted-foreground\">{suspicious.length} flagged entities</span>
                      <Badge variant=\"destructive\" className=\"text-[9px] px-1.5 py-0\">{suspicious.filter((s) => s.severity === \"HIGH\").length} HIGH</Badge>
                      <Badge variant=\"secondary\" className=\"text-[9px] px-1.5 py-0 bg-amber-500/10 text-amber-400\">{suspicious.filter((s) => s.severity === \"MEDIUM\").length} MEDIUM</Badge>
                    </div>
                    {suspicious.map((s, i) => (
              ...
              <Card className=\"border-border/40 bg-card/60\" data-testid=\"data-preview-table\">
                <CardHeader className=\"p-4 pb-2\">
                  <div className=\"flex items-center justify-between\">
                    ...
                    <Badge variant=\"secondary\" className=\"text-[9px]\">{preview.dataset_type} - {preview.record_count} rows</Badge>
                  </div>
                </CardHeader>
                      ...
                      <TableHeader>
                        <TableRow>
                          {(preview.columns || []).map((col) => (
                            <TableHead key={col} className=\"text-[10px] whitespace-nowrap font-mono\">{col}</TableHead>
                          ))}
                        </TableRow>
                      </TableHeader>
                      <TableBody>
                        {(preview.raw_preview || []).slice(0, 30).map((row, i) => (
                          <TableRow key={i}>
                            {(preview.columns || []).map((col) => (
                              <TableCell key={col} className=\"text-[11px] font-mono whitespace-nowrap py-1.5 max-w-[150px] truncate\">{row[col] || \"-\"}</TableCell>
                            ))}
                          </TableRow>
                        ))}
                      </TableBody>
